...
static uint8_t QSPI_Send_CMD(QSPI_HandleTypeDef *hqspi, uint32_t instruction, uint32_t address, uint32_t addressSize,
...
uint16_t w25qxx_GetID(void) {
    ...
    if (w25qxx_Mode == w25qxx_SPIMode)
        QSPI_Send_CMD(&hqspi, W25X_QUAD_ManufactDeviceID, 0x00, QSPI_ADDRESS_24_BITS, 6, QSPI_INSTRUCTION_1_LINE,
                      QSPI_ADDRESS_4_LINES, QSPI_DATA_4_LINES, sizeof(ID));
    else
        QSPI_Send_CMD(&hqspi, W25X_ManufactDeviceID, 0x00, QSPI_ADDRESS_24_BITS, 0, QSPI_INSTRUCTION_4_LINES,
                      QSPI_ADDRESS_4_LINES, QSPI_DATA_4_LINES, sizeof(ID));
...
uint8_t w25qxx_ReadSR(uint8_t SR) {
    uint8_t byte = 0;
    if (w25qxx_Mode == w25qxx_SPIMode)
        QSPI_Send_CMD(&hqspi, SR, 0x00, QSPI_ADDRESS_8_BITS, 0, QSPI_INSTRUCTION_1_LINE, QSPI_ADDRESS_NONE,
                      QSPI_DATA_1_LINE, 1);
    else
        QSPI_Send_CMD(&hqspi, SR, 0x00, QSPI_ADDRESS_8_BITS, 0, QSPI_INSTRUCTION_4_LINES, QSPI_ADDRESS_NONE,
                      QSPI_DATA_4_LINES, 1);
...
uint8_t w25qxx_SetReadParameters(uint8_t DummyClock, uint8_t WrapLenth) {
    ...
    QSPI_Send_CMD(&hqspi, W25X_SetReadParam, 0x00, QSPI_ADDRESS_8_BITS, 0, QSPI_INSTRUCTION_4_LINES, QSPI_ADDRESS_NONE,
                  QSPI_DATA_4_LINES, 1);
...
uint8_t W25qxx_EraseSector(uint32_t SectorAddress) {
    ...
    if (w25qxx_Mode == w25qxx_SPIMode)
        result = QSPI_Send_CMD(&hqspi, W25X_SectorErase, SectorAddress, QSPI_ADDRESS_24_BITS, 0,
                               QSPI_INSTRUCTION_1_LINE, QSPI_ADDRESS_1_LINE, QSPI_DATA_NONE, 0);
    else
        result = QSPI_Send_CMD(&hqspi, W25X_SectorErase, SectorAddress, QSPI_ADDRESS_24_BITS, 0,
                               QSPI_INSTRUCTION_4_LINES, QSPI_ADDRESS_4_LINES, QSPI_DATA_NONE, 0);
...
uint8_t W25qxx_EraseBlock(uint32_t BlockAddress) {
    ...
    if (w25qxx_Mode == w25qxx_SPIMode)
        result = QSPI_Send_CMD(&hqspi, W25X_BlockErase, BlockAddress, QSPI_ADDRESS_24_BITS, 0, QSPI_INSTRUCTION_1_LINE,
                               QSPI_ADDRESS_1_LINE, QSPI_DATA_NONE, 0);
    else
        result = QSPI_Send_CMD(&hqspi, W25X_BlockErase, BlockAddress, QSPI_ADDRESS_24_BITS, 0, QSPI_INSTRUCTION_4_LINES,
                               QSPI_ADDRESS_4_LINES, QSPI_DATA_NONE, 0);
...
uint8_t W25qxx_EraseChip(void) {
    ...
    if (w25qxx_Mode == w25qxx_SPIMode)
        result = QSPI_Send_CMD(&hqspi, W25X_ChipErase, 0x00, QSPI_ADDRESS_8_BITS, 0, QSPI_INSTRUCTION_1_LINE,
                               QSPI_ADDRESS_NONE, QSPI_DATA_NONE, 0);
    else
        result = QSPI_Send_CMD(&hqspi, W25X_ChipErase, 0x00, QSPI_ADDRESS_8_BITS, 0, QSPI_INSTRUCTION_4_LINES,
                               QSPI_ADDRESS_NONE, QSPI_DATA_NONE, 0);
...
uint8_t W25qxx_PageProgram(uint8_t *pData, uint32_t WriteAddr, uint32_t Size) {
    ...
    if (w25qxx_Mode == w25qxx_SPIMode)
        result = QSPI_Send_CMD(&hqspi, W25X_QUAD_INPUT_PAGE_PROG_CMD, WriteAddr, QSPI_ADDRESS_24_BITS, 0,
                               QSPI_INSTRUCTION_1_LINE, QSPI_ADDRESS_1_LINE, QSPI_DATA_4_LINES, Size);
    else
        result = QSPI_Send_CMD(&hqspi, W25X_PageProgram, WriteAddr, QSPI_ADDRESS_24_BITS, 0, QSPI_INSTRUCTION_4_LINES,
                               QSPI_ADDRESS_4_LINES, QSPI_DATA_4_LINES, Size);
...
static uint8_t QSPI_EnterQPI(QSPI_HandleTypeDef *hqspi) {
    ...
    QSPI_Send_CMD(hqspi, W25X_EnterQSPIMode, 0x00, QSPI_ADDRESS_8_BITS, 0, QSPI_INSTRUCTION_1_LINE, QSPI_ADDRESS_NONE,
                  QSPI_DATA_NONE, 0);
