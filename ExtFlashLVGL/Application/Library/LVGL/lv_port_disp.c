...
//    lv_disp_draw_buf_init(&draw_buf_dsc_3, buf_2_1, buf_2_2, MY_DISP_HOR_RES * 10); /*Initialize the display buffer*/
    ...
    static lv_color_t buf_3_1[
            MY_DISP_HOR_RES * MY_DISP_VER_RES]; /*A screen sized buffer*/
    static lv_color_t buf_3_2[
            MY_DISP_HOR_RES * MY_DISP_VER_RES]; /*Another screen sized buffer*/
...
static uint8_t *color[MY_DISP_HOR_RES] = {0};
...
static void disp_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t *color_p) {
    ...
    ST7735_WriteData16(0x00);
    ST7735_WriteData16(area->x1 + ST7735_XSTART);
    ST7735_WriteData16(0x00);
    ST7735_WriteData16(area->x2 + ST7735_XSTART);

//    ST7725_WriteCommand_Data(ST7735_CASET, data, sizeof(data));

    // row address set
    data[1] = area->y1 + ST7735_YSTART;
    data[3] = area->y2 + ST7735_YSTART;
    ST7725_WriteCommand_Data(ST7735_RASET, data, sizeof(data));

    // write to RAM
    HAL_GPIO_WritePin(ST7735_DC_GPIO_Port, ST7735_DC_Pin, GPIO_PIN_SET);
    ST7725_WriteCommand_Data(ST7735_RAMWR, (uint8_t *) color_p, sizeof(uint16_t) * w * h);
    /*IMPORTANT!!!
